...
<div class="wrap">
	...
	<div class="redis-queue-health-status <?php echo $health[ 'overall' ] ? 'healthy' : 'unhealthy'; ?>">
		<h2><?php esc_html_e( 'System Health', 'redis-queue-demo' ); ?> <span class="status-indicator"></span></h2>
		<div class="health-details">
			<div class="health-item"><span
					class="label"><?php esc_html_e( 'Redis Connection:', 'redis-queue-demo' ); ?></span><span
					class="value <?php echo $health[ 'redis' ] ? 'connected' : 'disconnected'; ?>"><?php echo $health[ 'redis' ] ? esc_html__( 'Connected', 'redis-queue-demo' ) : esc_html__( 'Disconnected', 'redis-queue-demo' ); ?></span>
			</div>
			<div class="health-item"><span
					class="label"><?php esc_html_e( 'Database:', 'redis-queue-demo' ); ?></span><span
					class="value <?php echo $health[ 'database' ] ? 'ok' : 'error'; ?>"><?php echo $health[ 'database' ] ? esc_html__( 'OK', 'redis-queue-demo' ) : esc_html__( 'Error', 'redis-queue-demo' ); ?></span>
			</div>
		</div>
	</div>
	...
	<div class="redis-queue-controls">
		...
		<div class="control-buttons">
			<button type="button" class="button button-primary"
				id="trigger-worker"><?php esc_html_e( 'Trigger Worker', 'redis-queue-demo' ); ?></button>
			<button type="button" class="button"
				id="refresh-stats"><?php esc_html_e( 'Refresh Stats', 'redis-queue-demo' ); ?></button>
			<button type="button" class="button button-secondary"
				id="run-diagnostics"><?php esc_html_e( 'Run Diagnostics', 'redis-queue-demo' ); ?></button>
			<button type="button" class="button button-secondary"
				id="debug-test"><?php esc_html_e( 'Full Debug Test', 'redis-queue-demo' ); ?></button>
			<button type="button" class="button button-secondary"
				id="reset-stuck-jobs"><?php esc_html_e( 'Reset Stuck Jobs', 'redis-queue-demo' ); ?></button>
		</div>
